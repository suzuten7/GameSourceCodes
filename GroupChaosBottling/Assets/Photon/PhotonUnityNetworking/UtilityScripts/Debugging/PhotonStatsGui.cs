...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class PhotonStatsGui : MonoBehaviour
    {
        ...
        public void TrafficStatsWindow(int windowID)
        {
            ...
            MaxOut = Mathf.Max(MaxOut,gls.TotalOutgoingMessageCount / elapsedMs);
            MaxIn = Mathf.Max(MaxIn, gls.TotalIncomingMessageCount / elapsedMs);
            MaxSum = Mathf.Max(MaxSum, gls.TotalMessageCount / elapsedMs);
            string AveMax = string.Format("Out {0,4} | In {1,4} | Sum {2,4}", MaxOut, MaxIn,MaxSum);
            GUILayout.Label(total);
            GUILayout.Label(elapsedTime);
            GUILayout.Label(average);
            GUILayout.Label("AveMax");
            GUILayout.Label(AveMax);
