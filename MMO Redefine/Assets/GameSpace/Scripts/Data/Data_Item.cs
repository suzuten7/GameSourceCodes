...
namespace Datas
{
    ...
    using UnityEditor;
#endif
    [CreateAssetMenu(menuName = "DataCre/Item")]
    public class Data_Item : Data_SlotGID
    {
...
#if UNITY_EDITOR
        [CustomEditor(typeof(Data_Item))]
        class ItemDataEditor : Editor
        {
            private Data_Item data;
            private Data_Item_DBL dbl;

            private void OnEnable()
            {
                data = (Data_Item)target;
            ...
            public override void OnInspectorGUI()
            {
                ...
                while (prop.NextVisible(enterChildren))
                {
                    ...
                    {
                        case "Type":
                            data.Type = (int)(Enum_ItemType)EditorGUILayout.EnumPopup("Type", (Enum_ItemType)data.Type);
                            break;
                        default:
                ...
                if (gid == -1)
                {
                    ...
                    {
                        var dbls = dbl.DBL.Find(x => x.Type == (Enum_ItemType)data.Type);
                        if (dbls == null)
                        {
                            dbls = new Data_Item_DBL.DBL_Item { Type = (Enum_ItemType)data.Type, Datas = new System.Collections.Generic.List<Data_Item>() };
                            dbl.DBL.Add(dbls);
                        }
                ...
                {
                    // 登録済みの場合
                    EditorGUILayout.HelpBox("登録済み:" + (Enum_ItemType)type + "の" + index + "番目に存在します。", MessageType.None);
                    if (type != data.Type) EditorGUILayout.HelpBox("種類が異なります", MessageType.Warning);

                    if (GUILayout.Button("×登録解除"))
                    {
                        var dbls = dbl.DBL.Find(x => x.Type == (Enum_ItemType)type);
