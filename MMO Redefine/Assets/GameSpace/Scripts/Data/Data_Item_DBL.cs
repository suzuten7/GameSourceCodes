namespace Datas
{
    ...
    [CreateAssetMenu(menuName = "DataCre/Data_ItemList")]
    public class Data_Item_DBL : ScriptableObject
    {
        public List<DBL_Item> DBL;
        [System.Serializable]
        public class DBL_Item
        {
            public Enum_ItemType Type;
            public List<Data_Item> Datas;
#if UNITY_EDITOR
            [CustomPropertyDrawer(typeof(DBL_Item))]
            class DBL_Item_Drawer : PropertyDrawer
            {
...
#endif
        static Dictionary<Data_Item, int> DataToGIDDic = new();
        static Dictionary<int, Data_Item> GIDToDataDic = new();
        public int DataGetGID(Data_Item ItemD)
        {
            ...
            for (int i = 0; i < DBL.Count; i++)
            {
                ...
                if (index >= 0)
                {
                    var id = GIDMake(Enum_ItemID.Material, (int)DBL[i].Type, index);
        ...
        public Data_Item GIDGetData(int GID)
        {
            if (GID < (int)Enum_ItemID.Material || GID >= (int)Enum_ItemID.Material + (int)Enum_ItemID.Category) return null;
            if (GIDToDataDic.TryGetValue(GID, out var oData)) return oData;
            var index = GIDBrake(GID, out var itemCategory, out var itemType);
            var dbl = DBL.Find(x => x.Type == (Enum_ItemType)itemType);
